...
import { Loader2, Check, Zap, Play, Square, ChevronDown, ChevronUp, Sparkles, Table2, Image } from "lucide-react";
import { Section, WriterSettings } from "./types";

/** Detect figure and table placeholders generated by ZOE in section content */
function detectContentElements(content: string | null): { figures: string[]; tables: string[] } {
  if (!content) return { figures: [], tables: [] };
  const figures: string[] = [];
  const tables: string[] = [];
  // Match [FIGURE X: description] placeholders
  const figMatches = content.match(/\[FIGURE\s+\d+[:\s][^\]]*\]/gi) || [];
  figMatches.forEach(m => figures.push(m.replace(/^\[|\]$/g, "")));
  // Match Figure X: caption lines
  const figCaptionMatches = content.match(/^Figure\s+\d+\s*:/gim) || [];
  if (figCaptionMatches.length > figures.length) {
    figCaptionMatches.forEach((m, i) => { if (!figures[i]) figures.push(m.trim()); });
  }
  // Match Table X: caption lines
  const tblMatches = content.match(/^Table\s+\d+\s*:/gim) || [];
  tblMatches.forEach(m => tables.push(m.trim()));
  // Match markdown table presence (| col | col |)
  const hasMdTable = /\|.+\|.+\|/.test(content);
  if (hasMdTable && tables.length === 0) tables.push("Markdown table");
        ...
        {sections.map((s, idx) => {
          ...
          return (
            <div key={s.id} className={`${idx > 0 ? "border-t border-border/50" : ""}`}>
              ...
              {/* Expanded content preview with detected tables/figures */}
              {isExpanded && s.content && (
                <div className="px-4 pb-3 space-y-2">
                  {/* Tables and figures chips */}
                  {(() => {
                    const { figures, tables } = detectContentElements(s.content);
                    if (figures.length === 0 && tables.length === 0) return null;
                    return (
                      <div className="flex flex-wrap gap-1.5">
                        {figures.map((f, i) => (
                          <span key={`fig-${i}`} className="flex items-center gap-1 px-2 py-1 bg-dusty-purple/10 border border-dusty-purple/20 text-dusty-purple rounded-full text-[10px] font-medium">
                            <Image size={10} /> {f.length > 40 ? f.slice(0, 40) + "…" : f}
                          </span>
                        ))}
                        {tables.map((t, i) => (
                          <span key={`tbl-${i}`} className="flex items-center gap-1 px-2 py-1 bg-sage/10 border border-sage/20 text-sage rounded-full text-[10px] font-medium">
                            <Table2 size={10} /> {t}
                          </span>
                        ))}
                      </div>
